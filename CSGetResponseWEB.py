...
class GetResponseWEB(object):
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=self.THREAD_POOL_SIZE) as t:
            ...
            for url_id, url in enumerate(self.URL_LIST):
                tasklist.append(t.submit(self.__scan, url_id + 1, url, self.TRYAGAIN))# 提交 URL 扫描任务
    ...
    def __callback(self, url_id, url, state, response):
        self.completed_url += 1
    ...
    def __scan(self, url_id, url, tryagain):
        header = self.__get_header()
        response = None
        try:
            with httpx.Client(
                    headers=header, follow_redirects=self.ALLOW_REDIRECT,
                    timeout=self.TIMEOUT, verify=False, proxies=self.PROXY

            ) as client:
                response = client.get(url)
                # response.raise_for_status()  # 抛出 HTTPStatusError 异常，如果请求不成功

            state = response.status_code

            # 调用回调函数处理扫描结果
            self.__callback(url_id, url, state, response)
            # return url_id, url, state, response, progress

        except httpx.RequestError as e:
            if DEBUG:
                print(f"[ERROR] | [请求出错] | {url} | [原因{e}]")
            # 记录错误信息到错误报告中
            ...
            if DEBUG:
                print(f"[ERROR] | [未知错误] | {url} | [原因{e}]")
            # 记录错误信息到错误报告中
            # self.__errorreport(str(e))
            # 如果设置了重试标志，尝试再次扫描该 URL
            if tryagain:
                self.__scan(url_id, url, False)
            state = "未知错误"
            self.__callback(url_id, url, state, response)
