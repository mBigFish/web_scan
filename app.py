...
EXCEL_WRITE_PATH_RELATIVE = 'output.xlsx'
EXCEL_WRITE_PATH_ABSOLUTE = ''
...
class WorkerThread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.url_id = 0
        self.urls = []
        self.session = None  # 将session初始化为None
        self.loop = None  # 添加这一行
    ...
    def run(self):
        ...
        self.update_signal.emit(f"文件保存路径：{EXCEL_WRITE_PATH_ABSOLUTE}")
    ...
    async def get_request(self, url_id, url):
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            try:
                async with await session.get(url="http://" + url, headers=headers, timeout=10) as response:
                    try:
    ...
    def parse(self, task):
        ...
        if status_code:
            ...
            if status_code == 200 and title:
                ...
                if url in title and "官网首页" in title:
                    web_status = "域名出售"
            else:
                web_status = "异常网站"
...
class MyWindow(QWidget, Ui_widget):
    ...
    def bind(self):
        ...
        # 运行窗口提示
        self.operation_status_edit.setPlaceholderText("请选择Excel表格时，URL一列放在表格第一列，且前面无http或https，当程序运行"
                                                      "完成时，将会为您输出保存文件路径！")
